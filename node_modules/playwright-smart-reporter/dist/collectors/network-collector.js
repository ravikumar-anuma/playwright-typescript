...
class NetworkCollector {
     ...
     */
    async collectFromTrace(tracePath) {
        const result = {
            entries: [],
            totalRequests: 0,
            totalDuration: 0,
    ...
    shouldInclude(entry) {
        ...
        if (this.options.contentTypeFilter?.length) {
            const matches = this.options.contentTypeFilter.some(ct => entry.contentType?.toLowerCase().includes(ct.toLowerCase()));
            if (!matches)
                return false;
        }
